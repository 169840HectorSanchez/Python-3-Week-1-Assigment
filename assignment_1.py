...
# Crear lista con los 9 colores y ademas obtener las etiquetas
enhancer=ImageEnhance.Brightness(image) #not important
images=[]
etiquetas=[] #guarda las etiquetas
font = ImageFont.truetype("readonly/fanwood-webfont.ttf",75)
for i in range(3):
    print(valor_RGB[i])
    for j in (0.1,0.5,0.9): #Aqui los valores pasaran por las 3 intensidades
        valor_RGB = image.split() #This method returns a tuple of individual image bands from an image.
#Se crean 3 imagenes cada una con las bandas originales
#Red, Blue, Green
        #usar Point https://www.geeksforgeeks.org/python-pil-image-point-method/
        etiquetas.append('channel {} intensity {}'.format(i,j))
        foto=valor_RGB[i].point(lambda x:x*j)
        valor_RGB[i].paste(foto)
        foto_RGB=Image.merge(image.mode,valor_RGB)
        images.append(foto_RGB)

# create a contact sheet from different brightnesses
first_image=images[0]
contact_sheet=PIL.Image.new(first_image.mode, (first_image.width*3,first_image.height*3+3*85))
#los 85 son para tener un exceso para el cuadro de dibujo
#Sin los 85 no es ve el cuadro
x=0
y=0
dibujo=ImageDraw.Draw(contact_sheet)
for i,img in enumerate(images):
    # Lets paste the current image into the contact sheet
    contact_sheet.paste(img, (x, y) )
    dibujo.text((x,y+first_image.height+5), etiquetas[i], font=font)
    # Now we update our X position. If it is going to be the width of the image, then we set it to 0
    # and update Y as well to point to the next "line" of the contact sheet.
    if x+first_image.width == contact_sheet.width:
        x=0
        y=y+first_image.height+85 #lo mismo que lo de arriba
    else:
